...
export function Header() {
  ...
  return (
    <header
      className="sticky top-0 z-40 px-6 py-3.5 flex flex-wrap items-center justify-between gap-4 border-b transition-colors duration-300"
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <Link href="/" className="flex items-center gap-2.5 group">
          ...
          <span className="font-bold text-[15px] tracking-tight flex items-center gap-1.5" style={{ color: 'var(--text-primary)' }}>
            Clinic<span className="text-gcore-orange">AI</span>
            <span className="text-[9px] uppercase font-bold tracking-widest px-2 py-0.5 rounded-full gcore-badge">Edge</span>
          </span>
        </Link>
      </div>
      ...
      <div className="flex items-center">
        <div className="flex items-center gcore-card rounded-apple px-3.5 py-1.5 text-xs gap-2 transition-apple hover:border-gcore-orange/40">
          ...
          <select
            ...
            className="bg-transparent font-medium focus:outline-none cursor-pointer pr-4 appearance-none text-[12px]"
          ...
          <ChevronDown className="w-3.5 h-3.5 -ml-2 pointer-events-none" strokeWidth={1.5} style={{ color: 'var(--text-muted)' }} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        {/* Latency Badge */}
        <div className="hidden sm:flex items-center gap-2 gcore-card px-3 py-1 rounded-full text-[11px] font-medium">
          <span className="status-dot bg-gcore-orange animate-pulse" />
          <span style={{ color: 'var(--text-secondary)' }}>
            Latency{' '}
            <strong className="text-orange-400 font-semibold font-mono">~575ms</strong>
          </span>
        </div>
