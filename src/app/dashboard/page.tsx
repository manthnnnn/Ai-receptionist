...
export default function DashboardOverviewPage() {
  ...
  const kpiCards = [
    {
      ...
      suffix: '',
      accent: 'text-white',
      borderColor: 'border border-white/10 hover:border-gcore-orange/40',
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-fade-in text-white">
      {/* Title Banner */}
      <div className="gcore-card rounded-apple-xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border border-white/10 bg-[#080808]">
        <div>
          ...
          <div className="flex items-center gap-3 text-xs text-neutral-400 mt-1.5 font-medium">
            ...
            <span className="text-neutral-300">Carrier SIP / PSTN Ready</span>
          </div>
        </div>
      ...
      {/* 6 Clean Minimalist KPI Cards (Pure Black + White/Orange Accents) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {kpiCards.map((card) => (
          <div
            key={card.label}
            className={`gcore-card rounded-apple-xl p-5 flex flex-col justify-between bg-[#080808] ${card.borderColor}`}
          >
            <span className="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider">{card.label}</span>
            <div className="mt-2 mb-1">
              <span className={`text-[28px] font-bold tracking-tight ${card.accent}`}>
                {card.value}{card.suffix}
              </span>
            </div>
            <div className="flex items-center gap-1.5 text-xs font-medium pt-2 border-t border-white/[0.06]">
      ...
      {/* Live Engine Banner */}
      <div className="gcore-card rounded-apple-xl p-6 text-white flex flex-col md:flex-row md:items-center justify-between gap-6 border border-gcore-orange/30 shadow-gcore-glow relative overflow-hidden bg-[#080808]">
